...
## 6. Примеры использования для клиентов

### Frontend: таблица клиентов

```javascript
// Запрос списка для таблицы
const response = await fetch('/api/v1/customers?page=1&limit=20', {
  headers: {
    'Accept': 'application/vnd.api.customer.list+json'
  }
});
const customers = await response.json();
// customers[0].countryName доступно сразу, без дополнительных запросов
```

### Frontend: dropdown выбора клиента

```javascript
// Запрос минимальных данных для select
...
const customers = await response.json();
// customers = [{id: "...", name: "John Doe"}, ...]
```

### Frontend: форма редактирования

```javascript
// Запрос данных для формы (без системных полей)
...
// customer без countryId, createdAt, updatedAt - чистые данные для UI
```
...
Готово! Если возникнут вопросы при реализации — обращайтесь.
